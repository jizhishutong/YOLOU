...
import test_face2  # import test_face2.py to get mAP after each epoch
from models.experimental import attempt_load
from models.yolo import FaceDetectionModel as Model
from utils.autoanchor import check_anchors
from utils.face.face_datasets import create_dataloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0] and epoch > 20:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                results, maps, times = test_face2.test(opt.data,
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for conf, iou, save_json in ([0.25, 0.45, False], [0.001, 0.65, True]):  # speed, mAP tests
                results, _, _ = test_face2.test(opt.data,
                                                batch_size=total_batch_size,
                                                imgsz=imgsz_test,
                                                conf_thres=conf,
                                                iou_thres=iou,
