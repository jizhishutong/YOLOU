...
from utils.pose.datasets import create_dataloader
...
def test(data,
    ...
    if training:  # called by train_face2.py
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if kpt_label:
                scale_coords(img[si].shape[1:], predn[:, 6:], shapes[si][0], shapes[si][1], kpt_label=kpt_label,
            ...
            if nl:
                ...
                scale_coords(img[si].shape[1:], tbox, shapes[si][0], shapes[si][1],
                             kpt_label=False)  # native-space labels, kpt_label is set to False as we are dealing with boxes here
                if kpt_label:
                    tkpt = labels[:, 5:]
                    scale_coords(img[si].shape[1:], tkpt, shapes[si][0], shapes[si][1],
                                 kpt_label=kpt_label)  # native-space labels
        ...
        if plots and batch_i < 3000:
            ...
            plot_images(img, output_to_target(out), paths, f, names, kpt_label=kpt_label, steps=3,
                        orig_shape=shapes[si])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='test_face2.py')
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        # python test_face2.py --task study --data coco.yaml --iou 0.7 --weights yolov5s.pt yolov5m.pt yolov5l.pt yolov5x.pt
