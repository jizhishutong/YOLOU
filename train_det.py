...
Usage:
    $ python path/to/train_det.py --data coco128.yaml --weights yolov5s.pt --img 640  # from pretrained (RECOMMENDED)
...
import val_det  # for end-of-epoch mAP
from models.experimental import attempt_load
from models.yolo import DetectionModel as Model
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val_det.run(data_dict,
    ...
    if RANK in (-1, 0):
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val_det.run(data_dict,
                                                batch_size=batch_size // WORLD_SIZE * 2,
                                                imgsz=imgsz,
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        LOGGER.info(f'Hyperparameter evolution finished {opt.evolve} generations\n'
                    ...
                    f'Usage example: $ python train_det.py --hyp {evolve_yaml}')
